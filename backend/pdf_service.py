...
def draw_editable_text(page, x, y, text, field_name, fontsize=7.5, width=160, height=16):
    val = text.strip() if text else ""
    try:
        tw = max(width, len(val) * fontsize * 0.5 + 10)
        w = fitz.Widget()
        w.rect = fitz.Rect(x, y - height + 2, x + tw, y + 2)
...
def fill_pdf(input_pdf_path, output_pdf_path, data_dict):
    ...
    for data_key, field_label in header_map:
        ...
        if info:
            ...
            draw_editable_text(doc[page_idx], text_x, y + 16, str(val) if val else "", data_key, fontsize=9, width=width, height=18)
    ...
    for item_num, (section, page_idx, y_center) in ITEM_POSITIONS.items():
        ...
        draw_editable_text(page, 475, y_center + 3, note or "", f"note_{item_num}", fontsize=6.5, width=100, height=14)

    extra_notes = data_dict.get("extra_notes", "")
    if len(doc) > 6:
        draw_editable_text(doc[6], 60, 55, str(extra_notes)[:200] if extra_notes else "", "extra_notes", fontsize=8, width=480, height=18)
    ...
    for i in range(1, 6):
        ...
        if len(doc) > 6:
            y_pos = 44 + (i - 1) * 31
            draw_editable_text(doc[6], 60, y_pos + 12, str(concern_val)[:100] if concern_val else "", f"concern_{i}", fontsize=8, width=480, height=18)
